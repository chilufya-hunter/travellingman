...
class SolveTSPUsingACO:
    ...
    def run(self):
        print('начало : {0}'.format(self.mode))
        if self.mode == 'ACS':
            self._acs()
        elif self.mode == 'Элитарный':
            self._elitist()
        else:
            self._max_min()
        print('Ended : {0}'.format(self.mode))
        print('Последовательность : <- {0} ->'.format(' - '.join(str(self.labels[i]) for i in self.global_best_tour)))
        print('Общее расстояние, пройденное для завершения тура : {0}\n'.format(round(self.global_best_distance, 2)))
...
if __name__ == '__main__':
    ...
    acs.run()
    acs.plot()
    elitist = SolveTSPUsingACO(mode='Элитарный', colony_size=_colony_size, steps=_steps, nodes=_nodes)
    elitist.run()
    elitist.plot()
